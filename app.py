...
# Page Config (Modern Dark Dashboard Style)
st.set_page_config(
...
st.markdown("""
    ...
    /* Button Customization with Neon Gradient Hover */
    .stButton>button { 
        width: 100%; 
        border-radius: 6px; 
        height: 3.2em; 
...
# Process Trigger Execution Block
if st.button("Analyze & Audit Transactions"):
    if not query:
        st.warning("⚠️ Input prompt sequence empty. Please enter a valid query string.")
    else:
        with st.spinner("🧠 Initializing Secure Vault Pipeline... Contextualizing data and running Reflective verification..."):
            try:
                # Target Gateway API Payload
                payload = {
                    "query": query,
                    "user_id": user_id,
                    "category": category
                ...
                if response.status_code == 200:
                    ...
                    # Split Dynamic View Layout
                    col1, col2 = st.columns([5, 2], gap="large")
                    
                    with col1:
                        st.markdown("<h3 style='color: #f0f6fc;'>📋 Core Diagnostics Report</h3>", unsafe_allow_html=True)
                        # Box wrapper for agent narrative output
                        st.markdown(f"""
                        <div style='background-color: #161b22; padding: 25px; border-radius: 8px; border: 1px solid #30363d; line-height: 1.6;'>
                            {data["analysis"]}
                        </div>
                        """, unsafe_allow_html=True)
                    ...
                    with col2:
                        ...
                        # Custom Graphical Metric Card 1: User Tenant Context
                        st.markdown(f"""
                        ...
                        # Custom Graphical Metric Card 2: Functional Filter
                        st.markdown(f"""
                            <div class="metric-card">
                        ...
                        # Custom Graphical Metric Card 3: Cache Performance State
                        cache_color = "#17a2b8" if data["is_cached"] else "#dc3545"
                        cache_text = "HIT (Saved Tokens)" if data["is_cached"] else "MISS (Full Compute)"
                        st.markdown(f"""
                            <div class="metric-card">
...
st.divider()
st.markdown(
    "<p style='text-align: center; color: #8b949e; font-size: 0.85rem;'>Sentio Vault Engine v2.0.0 • Production Level Cryptographic Security Sandbox Node • 2026</p>", 
    unsafe_allow_html=True
)